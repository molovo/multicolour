...
const debug = require("debug")("multicolour:handlers")
...
class Multicolour_Route_Templates {
  ...
  static compile_constraints(request, constraints) {
    ...
    Object.keys(constraints).forEach(key => {
      ...
      if (constraint instanceof Function) {
        query[key] = constraint.call(this, request, constraints)
      }
      // Check if it's a compilable constraint.
      else if (!constraint.compile && constraint.hasOwnProperty("value")) {
        query[key] = constraint.value
      }
      // It's probably something for jsocrud to handle then.
      else {
        query[key] = jsocrud.get(request, constraint.value || constraint, null)
      }
    })

    // Debugging.
    debug("Constraints compiled as %s", JSON.stringify(query, null, 2))
  ...
  static POST(request, callback) {
    ...
    if (this.constraints && this.constraints.post) {
      const constraints = Multicolour_Route_Templates.compile_constraints.call(this, request, this.constraints.post)
  ...
  static GET(request, callback) {
    ...
    if (this.constraints && this.constraints.get) {
      const constraints = Multicolour_Route_Templates.compile_constraints.call(this, request, this.constraints.get)
  ...
  static PATCH(request, callback) {
    ...
    if (this.constraints && this.constraints.patch) {
      const constraints = Multicolour_Route_Templates.compile_constraints.call(this, request, this.constraints.patch)
  ...
  static PUT(request, callback) {
    ...
    if (this.constraints && this.constraints.put) {
      const constraints = Multicolour_Route_Templates.compile_constraints.call(this, request, this.constraints.put)
  ...
  static DELETE(request, callback) {
    ...
    if (this.constraints && this.constraints.delete) {
      const constraints = Multicolour_Route_Templates.compile_constraints.call(this, request, this.constraints.delete)
    ...
    this.findOne(qs, (err, model) => {
      if (err) {
        ...
        callback(new Http_Error("Document(s) not found.", 410), null, this)
      }
      else {
        this.destroy(qs, err => {
          if (err) {
