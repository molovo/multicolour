...
class CLI_Init {
  ...
  deal_with_init_answers(answers) {
    ...
    if (answers.authentication.length > 0)
      // If auth was enabled
      return this.get_oauth_config(answers)
        .then(() => this.ask_for_default_user(answers))
        .then(update_answers => this.run_all_tasks(update_answers))
    ...
    return inquirer.prompt(providers.reduce((questions, provider) => {
      ...
      .then(provider_answers => {
        answers.auth = {
          ...
          providers: providers.map(strategy => {
            return {
