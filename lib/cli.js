...
class cli extends Map {
        ...
        plugins
          .map(plugin => {
            if (typeof plugin === "string" && !plugin.match(/^\.{0,2}?\//gi))
              return this.resolve_relative_path("/node_modules/" + plugin)
            else if (typeof plugin === "string" && plugin.match(/^\.{0,2}?\//gi))
              return this.resolve_relative_path(plugin)
            else if (plugin.name.match(/^\.{0,2}?\//gi)) {
              plugin.name = this.resolve_relative_path(plugin.name)
              return plugin
            }
            else return plugin
          })
          .forEach(plugin => {
            if (typeof plugin === "string") {
              this.debug(`Registering plugin "${plugin}"`)
              service.use(require(plugin))
            }
            else {
              // Register the plugin.
              service.use(require(plugin.name))

              // Register the sub plugins.
              plugin.plugins
                .map(sub_plugin => sub_plugin.match(/^\.{0,2}?\//gi) ? this.resolve_relative_path(sub_plugin) : sub_plugin)
                .forEach(sub_plugin => {
                  this.debug(`Registering plugin "${sub_plugin}" to "${plugin.as || plugin.name}"`)
                  service.get(plugin.as || plugin.name).use(require(sub_plugin))
                })
            }
          })
    ...
    inquirer.prompt([
      ...
      },
      {
        ...
        choices: [
          ...
          "Phabricator", "BitBucket", "Reddit", "Tumblr", "JWT"
        ]
      }
  ...
  write_package_json_file(answers) {
    ...
    if (answers.seed) user_package.dependencies["multicolour-seed"] = "^0.1.0"

    // Add the (currently) only supported server plugin.
    user_package.multicolour.plugins["multicolour-server-hapi"] = {
      as: "server",
    ...
    // Add the description.
    user_package.description = answers.description
    ...
    if (answers.authentication.indexOf("JWT") !== 1) {
      ...
      user_package.multicolour.plugins["multicolour-server-hapi"].plugins.push("multicolour-auth-jwt")
    }
    else if (answers.authentication.length > 0) {
      user_package.dependencies["multicolour-auth-oauth"] = "^1.0.0"
      user_package.multicolour.plugins["multicolour-server-hapi"].plugins.push("multicolour-auth-oauth")
    }
  ...
  _deal_with_init_answers(answers) {
    ...
    .then(() => {
      ...
      console.log(chalk.blue(`
        ...
        This includes a config file and a content/ folder. Please run
        ${chalk.underline(`cd ${this.write_path} && npm install`)} to install all dependencies.

        Running ${chalk.underline("npm start")} or ${chalk.underline("multicolour start")} will start running your server once dependencies are installed.
