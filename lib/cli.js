...
program.on("init", () => {
  ...
  fs
    ...
    .pipe(fs.createWriteStream(`${cwd}/config.js`))

  // Copy the package.json
  fs
    .createReadStream(path.resolve(`${__dirname}/../init/package.json`))
    .pipe(fs.createWriteStream(`${cwd}/package.json`))

  // Copy the default blueprint into place.
  ncp(path.resolve(`${__dirname}/../init/content`), `${cwd}/content`, err => {})

  // Emit a message, don't be a stranger yo.
