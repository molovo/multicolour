...
class Waterline_Generator {
  /**
   * Instantiated by Multicolour to create database models.
   * @param  {Array} blueprints array to generate from.
   * @param  {Object} api_config from the config file powering Multicolour.
   * @return {Multicolour_Server_Hapi} For immediate object return.
   */
  constructor(blueprints, api_config) {
  ...
  generate_model(blueprint) {
    ...
    // Add the schema to the stash.
    this.request("stash").set(name, collection)
  ...
  start(callback) {
    this.request("waterline").initialize(api_config, callback)
  }
}
...
module.exports = host => {
  return {
    ...
    type: host.request("types").DATABASE_GENERATOR,

    // The generator is the class above.
    generator: Waterline_Generator,

    // What part of the config do we want?
    config_key: "db"
  }
}
