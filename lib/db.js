...
class Multicolour_Waterline_Generator extends Map {
  ...
  start(callback) {
    ...
    const kill_sails_memory_adapter_with_unholy_fire = require("./utils").kill_sails_memory_adapter_with_unholy_fire
    const multicolour = this.request("host")
    const waterline = this.get("waterline")
    const config = multicolour.get("config").get("db")

    // Kill badly written adapters.
    this.debug("Killing badly written adapters out of love. 🔥")
    kill_sails_memory_adapter_with_unholy_fire(multicolour)
