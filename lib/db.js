...
class Multicolour_Waterline_Generator extends Map {
   ...
   * @return {Multicolour_Waterline_Generator} instance for chaining.
   */
  constructor() {
    // Construct.
    super()
  ...
  /**
   * Oh goody, this has to exist.
   *
   * @param {String} adapter identity to run fixes for.
   * @param {Object} collections to run fixes for.
   * @return {Promise} promise.
   */
  fix_waterline(fixes_for, collections) {
    const fixes = require("./waterline-fixes")

    if (!fixes.hasOwnProperty(fixes_for))
      return Promise.resolve()
    else
      return Promise.all(fixes[fixes_for].map(fix => fix(collections)))
  }

  /**
   * Start the database.
   * Also runs various fixes for known issues in various adapters.
   *
  ...
  start(callback) {
    ...
    this.get("waterline").initialize(config, (err, ontology) => {
      ...
      // Fix known issues with Waterline.
      if (!multicolour.get("power_user")) {
        const fixes_for = ontology.connections.development._adapter.identity
        this.fix_waterline(fixes_for, ontology.collections)
          .then(finalise, callback)
          .catch(err => {
            /* eslint-disable */
            /* istanbul ignore next: Untestable */
            console.error(err)
  ...
  /**
   * Run tear down routines on any adapters
   * and stop accepting queries then close
   * all connections.
   *
   * @param {Function} cb to run when all connections are closed.
   * @return {Multicolour_Waterline_Generator} instance for chaining.
   */
  stop(cb) {
    const multicolour = this.request("host")
