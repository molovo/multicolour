...
function convert(object) {
  ...
  const banned_update_create_keys = ["updatedAt", "createdAt", "id", "_id"]
  ...
  Object.keys(object).forEach(property_name => {
    // Get the property.
    const prop = object[property_name]
    ...
    if (prop instanceof Function) {
      key_out[property_name] = key_out[property_name].func(prop)
    }
    // If it's just a type, just do that.
    else if (!isObject(prop)) {
      key_out[property_name] = key_out[property_name][prop]()
    }
    ...
    if (prop.required === true) {
      key_out[property_name] = key_out[property_name].required()
    }
